...
make_target() {
  local variant uboot out dir uboot_dir kconfig defconfig type mstr timings clk sym val cflags c

  : >${PKG_BUILD}/.stub-keys
  for variant in ${PKG_STUB_VARIANTS}; do
    uboot="${variant%:*}"; out="${variant##*:}"; dir="${PKG_BUILD}/${out%.bin}"
    uboot_dir="$(get_build_dir ${uboot})"
    ...
    ( cd ${dir}
      cflags="-I$(get_build_dir atf)/plat/allwinner/sun50i_h616/include -I${PKG_BUILD}/compat -Iboards \
